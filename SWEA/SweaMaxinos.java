...
public class SweaMaxinos {
    ...
    private static void connectCore(int lv, int connected, int wireLen, int[][] inputMaxinos) {
        if (lv == cores.size()) {
        ...
        connectCore(lv + 1, connected, wireLen, inputMaxinos);
        ...
        for (int k = 0; k < 4; k++) {
            ...
            while (0 <= mi && mi < N + 2 &&
                0 <= mj && mj < N + 2 &&
                inputMaxinos[mi][mj] == 0) {
                mi += di[k];
                mj += dj[k];
            }
            if (inputMaxinos[mi][mj] == 9) {
                int[][] copyMaxinos = new int[N + 2][N + 2];
                for (int ci = 0; ci < N + 2; ci++) {
                    for (int cj = 0; cj < N + 2; cj++) {
                        copyMaxinos[ci][cj] = inputMaxinos[ci][cj];
                    }
                }
                mi = i + di[k];
                mj = j + dj[k];
                ...
                connectCore(lv + 1, connected + 1, wireLen + tempLen, copyMaxinos);
            }
        }
    }
}